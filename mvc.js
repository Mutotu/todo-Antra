...
const View = (function () {
  ...
  function isContentRowOrItem(element) {
    return (
      element.classList.contains("todolist__content__row") ||
...
const Controller = (function (view, model) {
  ...
  function setUpEvent() {
    document
      .querySelector(view.domSeletors.inputBarSubmit)
    ...
    document
      .querySelector(view.domSeletors.todolistContent)
      .addEventListener("click", (e) => {
        if (view.isDeleteButton(e.target)) {
          ...
          let id = view.getTodoIdFromElement(e.target)
            ? view.getTodoIdFromElement(e.target)
            : view.getTodoIdFromParent(e.target);
  ...
  function init() {
    // init
    view.renderHeader(model.headerConifg.title, model.headerConifg.submitText);
